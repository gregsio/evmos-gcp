...
command -v sponge >/dev/null 2>&1 || {
	echo >&2 "moreutils not installed."
	exit 1
}
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Testnet add-ons
	# Fetch testnet related configs from evmos/testnets repo
	# https://github.com/evmos/testnets/tree/main/evmos_9000-4

	# Download and unzip the evmos_9000-4 testnet genesis.zip file
	genesis_url="https://github.com/evmos/testnets/raw/main/evmos_9000-4/genesis.zip?download="
	genesis_file="${HOMEDIR}/config/genesis.zip"
	wget "$genesis_url" -O "$genesis_file"
	unzip -o "$genesis_file" -d "${CONFIGDIR}"
	...
	#	 - Uses sponge from moreutils package to minimze IO operations on large genesis file
	#	   reads the standard input and writes it out to the specified file, to reduce the number of file operations:
	# jq '.app_state["staking"]["params"]["bond_denom"]="aevmos"' "$GENESIS" | sponge "$GENESIS"
	# jq '.app_state["crisis"]["constant_fee"]["denom"]="aevmos"' "$GENESIS" | sponge "$GENESIS"
	# jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="aevmos"' "$GENESIS" | sponge "$GENESIS"
	# # When upgrade to cosmos-sdk v0.47, use gov.params to edit the deposit params
	# jq '.app_state["gov"]["params"]["min_deposit"][0]["denom"]="aevmos"' "$GENESIS" | sponge "$GENESIS"
	# jq '.app_state["evm"]["params"]["evm_denom"]="aevmos"' "$GENESIS" | sponge "$GENESIS"
	# jq '.app_state["inflation"]["params"]["mint_denom"]="aevmos"' "$GENESIS" | sponge "$GENESIS"
	...
	/usr/local/bin/genesis_edit.py $GENESIS
